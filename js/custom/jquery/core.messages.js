...
function junotoys_message(opt) {
    ...
    jQuery('body .junotoys_message').animate({
        top: top + Math.round((jQuery(window).height() - jQuery('.junotoys_message').height()) / 2),
        opacity: 1
    }, {
        complete: function() {
            // Call init function
...
function junotoys_message_destroy() {
    ...
    jQuery('.junotoys_message').animate({
        top: top - jQuery('.junotoys_message').height(),
        opacity: 0
    });
    setTimeout(function() {
        jQuery('#junotoys_modal_bg').remove();
