...
function junotoys_ready_actions() {
    ...
    jQuery('.widget_calendar').on('click', '.month_prev a, .month_next a', function(e) {
        ...
        jQuery.post(JUNOTOYS_STORAGE['ajax_url'], {
            ...
            try {
                ...
                rez = {
                    error: JUNOTOYS_STORAGE['ajax_error']
                };
    ...
    jQuery('.woocommerce,.woocommerce-page').on('click', '.mode_buttons a', function(e) {
        ...
        jQuery.cookie('junotoys_shop_mode', mode, {
            expires: 365,
            path: '/'
        });
        ...
        if (!exists) {
            var message_popup = junotoys_message_dialog('<label for="bookmark_title">' + JUNOTOYS_STORAGE['strings']['bookmark_title'] + '</label><br><input type="text" id="bookmark_title" name="bookmark_title" value="' + title + '">', JUNOTOYS_STORAGE['strings']['bookmark_add'], null,
                function(btn, popup) {
                    ...
                    setTimeout(function() {
                        junotoys_message_success(JUNOTOYS_STORAGE['strings']['bookmark_added'], JUNOTOYS_STORAGE['strings']['bookmark_add']);
                    }, JUNOTOYS_STORAGE['message_timeout'] / 4);
                });
        } else
...
function junotoys_init_appended_isotope(posts_container, filters) {
    ...
    if (!junotoys_check_images_complete(posts_container) && JUNOTOYS_STORAGE['isotope_init_counter']++ < 30) {
        setTimeout(function() {
            junotoys_init_appended_isotope(posts_container, filters);
        }, 200);
        return;
    }
    // Add filters
...
function junotoys_comments_validate(form) {
    ...
    var rules = {
        ...
        rules: [{
            ...
            min_length: {
                value: 1,
                message: JUNOTOYS_STORAGE['strings']['text_empty']
            },
            max_length: {
                value: JUNOTOYS_STORAGE['comments_maxlength'],
                message: JUNOTOYS_STORAGE['strings']['text_long']
            }
        }]
    };
    if (form.find('.comments_author input[aria-required="true"]').length > 0) {
        rules.rules.push({
            ...
            min_length: {
                value: 1,
                message: JUNOTOYS_STORAGE['strings']['name_empty']
            },
            max_length: {
                value: 60,
                message: JUNOTOYS_STORAGE['strings']['name_long']
            }
        });
    }
    if (form.find('.comments_email input[aria-required="true"]').length > 0) {
        rules.rules.push({
            ...
            min_length: {
                value: 7,
                message: JUNOTOYS_STORAGE['strings']['email_empty']
            },
            max_length: {
                value: 60,
                message: JUNOTOYS_STORAGE['strings']['email_long']
            },
            mask: {
                value: JUNOTOYS_STORAGE['email_mask'],
                message: JUNOTOYS_STORAGE['strings']['email_not_valid']
            }
        });
    }
...
function junotoys_login_validate(form) {
    ...
    var error = junotoys_form_validate(form, {
        ...
        rules: [{
                ...
                min_length: {
                    value: 1,
                    message: JUNOTOYS_STORAGE['strings']['login_empty']
                },
                max_length: {
                    value: 60,
                    message: JUNOTOYS_STORAGE['strings']['login_long']
                }
            },
            {
                field: "pwd",
                min_length: {
                    value: 4,
                    message: JUNOTOYS_STORAGE['strings']['password_empty']
                },
                max_length: {
                    value: 30,
                    message: JUNOTOYS_STORAGE['strings']['password_long']
                }
            }
        ]
    });
...
function junotoys_registration_validate(form) {
    ...
    var error = junotoys_form_validate(form, {
        ...
        rules: [{
                ...
                state: {
                    value: 'checked',
                    message: JUNOTOYS_STORAGE['strings']['not_agree']
                },
            },
            {
            ...
            },
            {
                ...
                equal_to: {
                    ...
                    message: JUNOTOYS_STORAGE['strings']['password_not_equal']
                }
            }
        ]
    });
