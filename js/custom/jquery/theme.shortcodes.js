...
function junotoys_sc_init_scroll_area(obj) {
    ...
    if (!junotoys_check_images_complete(obj) && JUNOTOYS_STORAGE['scroll_init_counter']++ < 30) {
        setTimeout(function() {
            junotoys_sc_init_scroll_area(obj);
        }, 200);
        return;
    }
        ...
        var o = {
            ...
            diagram: function() {
                ...
                r.circle(c, c, Math.round(w / 2)).attr({
                    stroke: 'none',
                    fill: JUNOTOYS_STORAGE['theme_bg_color'] ? JUNOTOYS_STORAGE['theme_bg_color'] : '#ffffff'
                });
    ...
    if (!form_custom) {
        ...
        if (form.find('[name="subject"]').length > 0) {
            rule = {
                ...
                max_length: {
                    value: 100,
                    message: JUNOTOYS_STORAGE['strings']['subject_long']
                }
            };
            if (form.find('[name="subject"][aria-required="true"]').length > 0)
                rule['min_length'] = {
                    value: 1,
                    message: JUNOTOYS_STORAGE['strings']['subject_empty']
                };
            rules.push(rule);
        }
        ...
        if (form.find('[name="message"]').length > 0) {
            rule = {
                field: "message",
                max_length: {
                    value: JUNOTOYS_STORAGE['contacts_maxlength'],
...
function junotoys_menuitems_show_popup(obj) {
    ...
    jQuery.post(JUNOTOYS_STORAGE['ajax_url'], {
        ...
        try {
            ...
            rez = {
                error: JUNOTOYS_STORAGE['ajax_error'] + '<br>' + response
            };
        }
